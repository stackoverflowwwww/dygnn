...
def get_args():
    ...
    if flag==0:
        parser.add_argument('-m', '--model_file', type = str,default="saved_models/UCI/nt_0_wd_0.001_up_0_w_2.0_b_200_l_0.001_tr_0.8_nn_1_tanh_trans_1_dr_p_0_prop_1_att_1_rp_1_dcm_log_nor_0/model_after_epoch_3.pt",
                            help = 'DyGNN model file')
    elif flag==1:
        # parser.add_argument('-m', '--model_file', type = str,default="saved_models/DNC/nt_0_wd_0.001_up_0_w_2_b_200_l_0.001_tr_0.8_nn_5_tanh_trans_1_dr_p_0_prop_1_att_1_rp_1_dcm_log_nor_1_th_20/model_after_epoch_3.pt",
        #                     help = 'DyGNN model file')
        # parser.add_argument('-m', '--model_file', type = str,default="saved_models\\DNC2\\model_after_epoch_3.pt",
        #                     help = 'DyGNN model file')
        parser.add_argument('-m', '--model_file', type = str,default="saved_models\\DNC3\\nt_0_wd_0.001_up_0_w_2_b_200_l_0.001_tr_0.8_nn_5_tanh_trans_1_dr_p_0.0_prop_1_att_1_rp_1_dcm_log_nor_0_th_20\\model_after_epoch_1.pt",
                            help = 'DyGNN model file')
                        
    parser.add_argument('-tr', '--train_ratio', type = float,
                        help = 'train_ratio', default = 0.8)
    parser.add_argument('-vr', '--valid_ratio', type = float,
                        help = 'valid_ratio', default = 0.01) #0.01 
...
def myTest(args,num_nodes, data):
    ...
    if int(len(data) * (train_ratio + valid_ratio + test_ratio)) > len(data):
        ...
        test_data = data[int(len(data) * (train_ratio + valid_ratio)):int(
    ...
    print('Train End Time: ', str(train_end_time), '; Test Start Time: ', str(test_start_time), 'Test End Time ',
          str(test_end_time), '; Gap: ', str(test_end_time - test_start_time))
    ...
    dyGnn.eval()
    dyGnn.load_state_dict(torch.load(model_file), strict=False)
    ...
    for interaction in test_data:
        ...
        head_tensor=head_reps(torch.LongTensor([int(interaction[0][0])]).to(device))
        tail_tensor=tail_reps(torch.LongTensor([int(interaction[0][1])]).to(device)).view(-1,1)
        score=torch.mm(head_tensor,tail_tensor)
        if score>=0:
        ...
        i+=1
        dyGnn(interaction)
    return test_data,correct,error
...
def test(args,num_nodes, data):
    ...
    all_nodes = get_all_nodes(train_data)
    print(len(all_nodes))
    # candidate为map，，键为源节点或尾节点，值为全体训练节点
    ...
    # np.savetxt("log.txt",head_lengths_numpy)
...
def main():
    ...
    if args.dataset == 'uci':
        if flag==1:
            data = Temporal_Dataset('Dataset/email-dnc.edges',1,0)
            num_nodes = 2029
        elif flag==0:
            data = Temporal_Dataset('Dataset/UCI_email_1899_59835/opsahl-ucsocial/out.opsahl-ucsocial',1,2)
            num_nodes = 1899

        np.save("data_dnc.npy",data.data)
        # graph=nx.from_edgelist(data.data[:,0:2],create_using=nx.MultiDiGraph)
        print('Test on UCI dataset')
        # test(args, num_nodes, data)

        test_data,correct,error=myTest(args, num_nodes, data)
        np.save("test_data_dnc.npy",test_data)
        np.savetxt("correct_dnc.txt",correct,fmt="%d")
        np.savetxt("error_dnc.txt",error,fmt="%d")
