...
class DyGNN(nn.Module):
    def __init__(self, num_embeddings, embedding_dims, edge_output_size, device, w , is_att = False,transfer=False , nor =0, if_no_time=0, threhold = 20, second_order=False, if_updated = 0, drop_p = 0, num_negative = 5 , act = 'tanh', if_propagation = 1 ,decay_method='exp', weight = None, relation_size=None,bias = True):
        ...
        #todo
        # torch.nn.init.xavier_uniform(self.tran_head_edge_head.weight)
        # torch.nn.init.xavier_uniform(self.tran_head_edge_tail.weight)
        # torch.nn.init.xavier_uniform(self.tran_tail_edge_head.weight)
        # torch.nn.init.xavier_uniform(self.tran_tail_edge_head.weight)
        self.is_att = is_att
        if self.is_att:
        ...
        if transfer:
            ...
            #todo
            # torch.nn.init.xavier_uniform(self.transfer2head.weight)
            # torch.nn.init.xavier_uniform(self.transfer2tail.weight)
            if drop_p>=0:
