...
def get_args():
    ...
    parser.add_argument('-l', '--learning_rate', type = float,
                        help = 'learning_rate', default = 0.001) #原本值：0.001
    ...
    parser.add_argument('-act', '--act', type = str,
                        help = 'act function', default = 'tanh') #tanh
    ...
    parser.add_argument('-dp' , '--drop_p', type = float,
                        help = 'dropout_rate', default = 0.0) #0
    ...
    parser.add_argument('-w', '--w', type = float,
                        help = 'w for decayer', default = 2) #原本值：2
    parser.add_argument('-s', '--seed', type = int,
                        help = 'random seed', default = 0) #原本值：0
    ...
    parser.add_argument('-nor', '--nor', type = int ,
                        help = 'normalize or not', default = 0) #0
    ...
    parser.add_argument('-th', '--threhold', type = float,
                        help = 'the threhold to filter the neighbors, if None, do not filter', default = 20)
...
def rank(node, true_candidate, node2candidate, node_reps, candidate_reps, device, pri = False):
    # node_reps和candidate_reps都为全部节点(embedding vector)
    ...
    rank = rankdata(negative_scores_numpy)[-1] #只取最后一个，即true_candidate
...
if __name__ == '__main__':
    ...
    if args.dataset == 'uci':
        if flag==1:
            ...
            num_nodes = 2029
            # model_save_dir = model_save_dir + 'DNC/'
        elif flag==0:
        ...
        model_save_dir = model_save_dir + 'DNC3/'
